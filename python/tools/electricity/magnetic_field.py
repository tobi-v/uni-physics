...
def HelmholtzAlongZ(
    z: ndarray, current: float, radius: float, distance: float, turns=1
) -> ndarray:
    ...
    Returns: B_z field (T) at position(s) z
    """
    return BOfCoilAlongZ(z + distance / 2, current, radius, turns) + BOfCoilAlongZ(
        z - distance / 2, current, radius, turns
    )
