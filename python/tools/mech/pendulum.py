...
def SchwerpunktsLaenge(T, uncertainty=False, delT=0):
    def SchwerpunktsLaengeInner(T):
        return g * T**2 / (4 * pi**2)

    return GetResultAndUncertainty(SchwerpunktsLaengeInner, [T], uncertainty, [delT])
...
def IfromPeriod(period, mass, length, uncertainty=False, delT=0, delM=0, delL=0):
    ...
    return GetResultAndUncertainty(
        IfromPeriodInner, [period, mass, length], uncertainty, [delT, delM, delL]
    )
...
def DegreeOfCouplingBeat(
    period, beat_period, uncertainty=False, delTperiod=0, delTbeat=0
):
    def DegreeOfCouplingBeatInner(period, beat_period):
        return 2 * period * beat_period / (period**2 + beat_period**2)

    return GetResultAndUncertainty(
        DegreeOfCouplingBeatInner,
        [period, beat_period],
        uncertainty,
        [delTperiod, delTbeat],
    )
